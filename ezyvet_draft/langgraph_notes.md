...
# Deployment terminology and information
...
This might be useful if we were deploying multiple apps to the same langgraph, especially if we weren't sure which languages we wanted to connect to it with. Also if we wanted multiple assistants, or we wanted to handle double texting. But every other part of the graph stays the same between deployments
http requests:
...
# Benefits of deploying your graph separately to backend
...
Notice that this application client.run.create() is for using an sdk to communicate with a deployed graph.
This is different to graph.invoke() which is for graphs you just write to your backend. If you are not using an sdk, then you have to
code in the logic yourself at the application layer.
...
## Assistants - different versions of the same graph

Different deployments of the same graph can have a configuration file in our deployment folder
This could include the userId of the client, the system prompt.
This is also done through sdk. Another advantage of deploying the app separately.
